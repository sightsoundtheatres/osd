...
Process {
    ...
    if ($AzureStorageBlobContent -ne $null) {
        ...
        # Check if screen wallpaper files are present in the Azure Storage blog and replace those in the default location
        $WallpaperBlobScreenFiles = $AzureStorageBlobContent | Where-Object { $PSItem.Name -match "^img10[0-5].jpg$" }
        if ($WallpaperBlobScreenFiles -ne $null) {
            Write-LogEntry -Value "Detected screen wallpaper files in container, will replace matching local screen file" -Severity 1
            # Define screen wallpaper path and retrieve all image files
            $ScreenWallpaperImagePath = Join-Path -Path $env:windir -ChildPath "Web\Screen"
            $ScreenWallpaperImages = Get-ChildItem -Path $ScreenWallpaperImagePath -Filter "*.jpg"
            foreach ($WallpaperBlobScreenFile in $WallpaperBlobScreenFiles) {
                # Remove current screen wallpaper image file and replace with image from storage account
                if ($WallpaperBlobScreenFile.Name -in $ScreenWallpaperImages.Name) {
                    Write-LogEntry -Value "Current container item with name '$($WallpaperBlobScreenFile.Name)' matches local wallpaper item, starting replacement process" -Severity 1
                    # Get matching local wallpaper image for current container item
                    $ScreenWallpaperImage = $ScreenWallpaperImages | Where-Object { $PSItem.Name -like $WallpaperBlobScreenFile.Name }
                    # Remove current screen wallpaper image file
                    Remove-WallpaperFile -FilePath $ScreenWallpaperImage.FullName
                    # Download new wallpaper content from storage account
                    Invoke-WallpaperFileDownload -FileName $WallpaperBlobScreenFile.Name -Destination $ScreenWallpaperImagePath
                } else {
                    Write-LogEntry -Value "Downloaded screen wallpaper with file name '$($WallpaperBlobScreenFile.Name)' doesn't match any of the built-in screen wallpaper image file names, skipping" -Severity 2
                }
            }
        }
    }
